<template>
    <div>{{ content }}</div>
        <div class='choices' v-for="option in options" :key="option.id">
            ...
            <input type="text" v-model="option.content" required=true :readonly="options[option.id-1].changeable" :placeholder="option.id.toString()">
...
export default {
  ...
  methods: {
    ...
    changeChoice (index) {
      this.options[index].changeable = !this.options[index].changeable
    }
  }
}
</script>
